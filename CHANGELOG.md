...
### Changed
- **The oam floor moves from 0.15.2 to 0.16.3, so an oam older than that now falls back to Node.** This server is verified on one oam release at a time, and the floor is what enforces that. 0.16.3 is verified: full MCP handshake, all 28 tools, and the documented `aws_script` sandbox divergence re-measured on it (inside the sandbox `eval('1+1')` returns 2 and `Function('return 7')()` returns 7 under oam, while both raise `EvalError` under Node). **If you are on oam 0.15.2 through 0.16.2, the launcher will now hand off to Node instead of serving on your oam** -- it says so on stderr, naming the version it found and the floor. Nothing about the server's behaviour changes when that happens; oam and Node run the same code and the same resolved AWS CLI. Update oam, or set `AWS_MCP_RUNTIME=node` to make the choice explicit.
- **`AWS_MCP_AWS_CLI` pointing at a Windows `aws.exe` is now refused when the server is not on Windows.** This is the WSL case, and it was a trap rather than a configuration: `/mnt/c/.../aws.exe` passes every other check (the path is absolute, the `.exe` rule is Windows-only, and DrvFs reports 0777 so the executable test is satisfied), and WSL interop really does run it -- so it worked until it didn't. The server is a POSIX process: it writes `/tmp/aws-mcp-input-*/params.json` and passes `file:///tmp/...`, and `aws_lambda_invoke` hands the CLI a POSIX outfile path. A Windows CLI resolves both against the current drive, so params over the inline cap cannot be read and an invoke response is written where nothing looks for it. The error names the binary, the platform, and which transports break. Install the Linux build and point this at that. (The override is the only way a Windows executable can be reached on POSIX -- the `PATH` walk looks for a file named `aws`, never `aws.exe`.)

### Fixed
- **A mis-spelled `AWS_MCP_AWS_CLI` no longer passes in silence on macOS and Linux.** Environment variables are case-sensitive there, so `aws_mcp_aws_cli=/usr/local/bin/aws` was not "the override set wrong", it was "no override at all": the `PATH` walk answered instead and returned ok, leaving the operator with a working server running a different binary than the one they configured. Measured end to end -- an override naming a real executable plus a `PATH` holding another `aws` resolved to the `PATH` one, with no diagnostic. That defeats the whole design of this variable, which is to fail loudly rather than fall back, because it picks which binary handles your credentials. A near-miss now prints one line naming the spelling found and the canonical one, once per spelling rather than once per call. Windows is unaffected: the lookup case-folds there, so the variable is honoured as written.
- **The `aws` child processes pass `windowsHide`, matching the launcher.** `bin/aws-mcp.mjs` set it on its version probe and both launch paths; the three spawns that run the actual AWS CLI did not, so libuv never added `CREATE_NO_WINDOW` for them. Bounded honestly: a console application inherits its parent's console, so a window could only appear where the server itself has none -- a GUI-subsystem MCP host launching it detached. Asserted rather than measured, because discriminating it needs a console-less parent plus a window probe. Inert on macOS and Linux.
