...
### Breaking
- **`aws_iam_simulate` no longer returns `evaluationResults`.** The full raw array was returned alongside the parsed `results`, doubling the payload for no consumer.
...
- **`runAwsCall` settles on `'close'`, not `'exit'`.** `'exit'` fires while stdio pipes may still hold unread data; `sso.ts` already used `'close'` with a comment explaining that pipes must flush first. Truncated stdout then failed `JSON.parse` and was returned as a successful call.
